function updateRidgeLine(selectedDataset_1, selectedDataset_2, selectedYears) {
    ...
    Promise.all([
        ...
        var x = d3.scaleLinear()
            .domain([minTemperature - 2, maxTemperature + 10])
            .range([0, width]);
        
        svg.append("g")
            .attr("class", "xAxis")
            .attr("transform", "translate(0," + height + ")")
            .call(d3.axisBottom(x).tickValues([minTemperature - 2, minTemperature, 0, maxTemperature, maxTemperature + 10]).tickSize(-height) )
