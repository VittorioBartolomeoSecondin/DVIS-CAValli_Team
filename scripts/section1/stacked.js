function updateStackedChart(selectedValue, useAlternateDataset) {
  ...
  d3.csv(datasetURL).then( function(data) {
    ...
    const color = d3.scaleOrdinal()
                    .range(['#b51d14', '#ddb310', '#4053d3', '#fb49b0', '#00b25d', '#cacaca']) //#cacaca for the 6th color
                    .domain(subgroups);
...
updateStackedChart("all", false); // Load the chart with all cities initially
...
document.getElementById("city-dropdown").addEventListener("change", function () {
    ...
    updateStackedChart(selectedValue, useAlternateDataset);
});
